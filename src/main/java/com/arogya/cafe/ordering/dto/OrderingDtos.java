...
public final class OrderingDtos {
    ...
    public record OrderLineResponse(
            Long id, Long menuItemId, String menuItemName, String sizeVariant,
            int quantity, BigDecimal unitPrice, BigDecimal lineTotal) {
        public static OrderLineResponse from(OrderLine l) {
            return new OrderLineResponse(l.getId(), l.getMenuItem().getId(), l.getMenuItem().getName(),
                    l.getSizeVariant(), l.getQuantity(), l.getUnitPrice(), l.getLineTotal());
        }
    }
    ...
    public record KotResponse(Long id, Long orderId, KotStatus status, Instant issuedAt, List<String> fulfilledBy) {
        public static KotResponse from(Kot k) {
            return new KotResponse(k.getId(), k.getOrder().getId(), k.getStatus(), k.getIssuedAt(),
                    k.getFulfilledBy().stream().map(Staff::getName).toList());
        }
    }

    public record BillResponse(Long id, Long orderId, BigDecimal totalAmount, PaymentStatus paymentStatus,
                               Instant billedAt) {
        public static BillResponse from(Bill b) {
            return new BillResponse(b.getId(), b.getOrder().getId(), b.getTotalAmount(), b.getPaymentStatus(),
                    b.getBilledAt());
        }
    }

    public record OrderResponse(
            Long id, OrderStatus status, Instant createdAt, Long customerId, String customerName,
            List<String> handledBy, List<OrderLineResponse> lines, KotResponse kot, BillResponse bill) {
        ...
        public static OrderResponse build(Order order, List<OrderLine> lines, Kot kot, Bill bill) {
            return new OrderResponse(
                    ...
                    KotResponse.from(kot), BillResponse.from(bill));
        }
    }
}
